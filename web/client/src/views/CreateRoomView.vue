...
onMounted(() => {
  store.disconnect();
  const prefilledSlug = typeof route.query.slug === 'string' ? route.query.slug : '';
  if (prefilledSlug) {
    createVanityUrl.value = prefilledSlug;
    checkSlugAvailability(prefilledSlug);
  } else {
...
<template>
  <div class="min-h-dvh flex items-center justify-center bg-gray-950 text-white p-6">
    <div class="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-md">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-xl font-semibold text-white">Create Room</h2>
        <button
